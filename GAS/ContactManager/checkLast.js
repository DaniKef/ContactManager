...
function checkLastCallAndBirthday()                                          // Выполнить проверку последнего звонка и дня рождения.
{
  ...
  var lastRowUsers = sheetUsers.getLastRow();
  for(var index = 1; index <= lastRowUsers; index = index + 1) 
  {
    try
    {
      ...
      if(holderForLastCallFriend.length > 0)    // Если хоть 1 человек в группе друзей.
      {
        // Отправить соответствующее чообщение на почту.
          MailApp.sendEmail(currentGmail, "Contact Manager", "Вы не звонили друзьям " + holderForLastCallFriend + " уже больше " + daysForFriend + " дней. Не забывайте делать звонки друзьям!");
      }

      if(holderForLastCallRelative.length > 0)  // Если хоть 1 человек в группе родственников.
      {
        // Отправить соответствующее чообщение на почту.
          MailApp.sendEmail(currentGmail, "Contact Manager", "Вы не звонили родственникам " + holderForLastCallRelative + " уже больше " + daysForRelative + " дней. Не забывайте делать звонки близким людям!");
      }

      if(holderForLastCallColleague.length > 0) // Если хоть 1 человек в группе коллег.
      {
        // Отправить соответствующее чообщение на почту.
          MailApp.sendEmail(currentGmail, "Contact Manager", "Вы не звонили коллегам " + holderForLastCallColleague + " уже больше " + daysForColleague + " дней. Не забывайте делать звонки коллегам!");
      }

      if(holderForBirthday.length > 0)          // Если хоть у 1 человека сегодня день рождения.  
      {
        // Отправить соответствующее чообщение на почту.
          MailApp.sendEmail(currentGmail, "Contact Manager", "Сегодня у " + holderForBirthday + " день рождения. Не забудьте поздравить с днем рождения!");
      }
