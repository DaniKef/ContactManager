  ...
  for(var index = 1; index <= lastRow; index = index + 1)                           // Пройтись по листу.
      {
        ...
        if(nam == e.parameter['name'] && e.parameter['oldName'] == "")                                            // Если телефон нужный, значит, контакт существет и его надо изменить.
          {
        ...
        else if(nam == e.parameter['oldName'])
        {
            sheet.getRange(index,1).setValue(e.parameter['name']); 
            sheet.getRange(index,2).setValue(e.parameter['company']); 
            sheet.getRange(index,3).setValue(e.parameter['group']); 
            sheet.getRange(index,4).setValue(new Date(e.parameter['birthday'])); 
            sheet.getRange(index,5).setValue(e.parameter['phone']); 
    ...
    for(var column = 1; column <= lastColumn; column = column + 1)                         // Пройтись по столбцам.     
    {
      if(sheet.getRange(1,column).getValue() == e.parameter['column'])                     // Если нашелся нужный столбец.
      {
          for(var row = 1; row <= lastRow; row = row + 1)                                  // Пройтись по ячейкам столбца.
          {
            ...
            if(sheet.getRange(1,column).getValue() == "Birthday" || sheet.getRange(1,column).getValue() == "Last Call")
            first = addZeroToDate(first);
...
function addZeroToDate(first)
{
  var dateee = new Date(first);
  first = "";
  //first = dateee.getDate().toString() + "." + (dateee.getMonth() + 1).toString() + "." + dateee.getFullYear().toString();
  if(dateee.getDate().toString().length != 2)
  first += "0" + dateee.getDate().toString() + ".";
  else
  first += dateee.getDate().toString() + ".";

  if((dateee.getMonth() + 1).toString().length != 2)
  first += "0" + (dateee.getMonth() + 1).toString() + ".";
  else
  first += (dateee.getMonth() + 1).toString() + ".";

  first += dateee.getFullYear().toString();
